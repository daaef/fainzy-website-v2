    ...
    <div className="w-full">
      ...
      >
        <div className="relative w-full h-[40vh] sm:h-[45vh] md:h-[50vh]">
          <Image src={robot.image} alt="ZiBot Model" fill className="object-contain w-full" />
        </div>
      </motion.div>
      ...
      >
        ...
        <p className="text-sm sm:text-base text-gray-400 mb-8 sm:mb-12 max-w-3xl">{description}</p>
      </motion.div>
        ...
        {pricingTiers.map((tier, index) => (
          ...
          >
            ...
            <div className="mb-4 flex flex-col sm:flex-row items-center sm:items-start gap-2 sm:gap-4 md:gap-8 lg:gap-16">
              <h3
                className={`text-lg sm:text-xl md:text-2xl font-bold ${tier.name === "First 100 Exclusive" ? "text-white animate-pulse" : "text-white"}`}
              >
                {tier.name}
              </h3>
              {tier.badge && (
