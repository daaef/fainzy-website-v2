  ...
  return (
    <div className="w-full">
      ...
      <div className="relative w-full h-[70vh] mx-auto mb-16">
        <Image src={robotImage} alt="ZiBot Top View" fill className="object-contain" />
      </div>
      ...
      <div className={gridClass}>
        {gridItems.map((item) => (
          <div key={item.id} className="flex flex-col">
            ...
            <h4
              className={`${gridCols === 3 ? "text-base" : "text-xs"} font-semibold text-white mb-2`}
            >
              {item.title}
            </h4>
            <p
              className={`${gridCols === 3 ? "text-sm" : "text-xs"} text-gray-400 leading-relaxed`}
            >
              {item.description}
            </p>
          </div>
